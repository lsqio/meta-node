...
class Meta extends EventEmitter {
  ...
  service(name) {
    if (this._service.has(name)
        && this._service.get(name).nodes
        && this._service.get(name).nodes.length > 0) {
  ...
  _watchConfig() {
    return this._call(`/v1/config?index=${this._configIndex}`, null, true)
      .then((res) => {
        const index = this._configIndex;
        const config = this._config;
        this._configIndex = parseInt(res.headers['x-index'], 10);
        this._config = res.body;
        this._watchConfig();

        if (index < this._configIndex
  ...
  _watchServices() {
    return this._call(`/v1/services?index=${this._servicesIndex}`, null, true)
      .then((res) => {
        const index = this._servicesIndex;
        const services = this._services;
        this._servicesIndex = parseInt(res.headers['x-index'], 10);
        this._services = res.body;
        this._watchServices();

        if (index < this._servicesIndex
            && JSON.stringify(services) !== JSON.stringify(this._services)) {
  ...
  _watchService(name) {
    const service = this._service.get(name);
    const index = service ? service.index : 0;

    return this._call(`/v1/services/${name}?index=${index}`, null, true)
      .then((res) => {
        const _index = parseInt(res.headers['x-index'], 10);
        const nodes = service ? service.nodes : [];
        this._service.set(name, { nodes: res.body, index: _index });
        this._watchService(name);
        if (index < _index
            && JSON.stringify(nodes) !== JSON.stringify(this._service.get(name).nodes)) {
  ...
  static _request(_optionsParam, options) {
    const _options = _optionsParam;
    return new Promise((resolve, reject) => {
      _options.headers = _options.headers ? _options.headers : {};
      if (options.json) _options.headers['Content-Type'] = 'application/json';

      const onRes = (_res) => {
        if (options.encoding) _res.setEncoding(options.encoding);

        const augment = (res, cb) => {
      ...
      const req = http.request(_options, onRes);
      req.on('error', reject);
